...
\restrict X6ZSD054oofY8a4N1CNRo4B82n1FgW0xafZLIdrZeaf2MSk4O6dPAzRAkdXkTNx
...
CREATE TABLE public.export_log (
    ...
    CONSTRAINT export_log_formato_chk CHECK (((formato)::text = ANY ((ARRAY['pdf'::character varying, 'csv'::character varying])::text[])))
);
...
COPY public.empleado (id, property_id, nombre, apellidos, dni_nie, fecha_nacimiento, telefono, email, puesto, departamento_id, fecha_alta, fecha_baja, activo, notas, creado_en, actualizado_en, pin_hash, pin_lookup, qr_token) FROM stdin;
\.
...
SELECT pg_catalog.setval('public.departamento_id_seq', 9, true);
...
SELECT pg_catalog.setval('public.empleado_id_seq', 3, true);
...
SELECT pg_catalog.setval('public.export_log_id_seq', 2, true);
...
SELECT pg_catalog.setval('public.fichaje_id_seq', 13, true);
...
\unrestrict X6ZSD054oofY8a4N1CNRo4B82n1FgW0xafZLIdrZeaf2MSk4O6dPAzRAkdXkTNx
