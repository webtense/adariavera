...
const USER='francesc', NOMBRE='Francesc (Dirección)', ROLE='lector', DIAS=365;
...
if(!u){ const h=bcrypt.hashSync(crypto.randomBytes(12).toString('base64url'),12);
  ...
  u=db.prepare('SELECT * FROM users WHERE username=?').get(USER); console.log('usuario francesc creado (id '+u.id+', rol '+ROLE+')'); }
else console.log('usuario francesc ya existe (id '+u.id+')');
