...
const USER='olga', NOMBRE='Olga (Dirección)', ROLE='lector', DIAS=365;
...
if(!u){ const h=bcrypt.hashSync(crypto.randomBytes(12).toString('base64url'),12);
  ...
  u=db.prepare('SELECT * FROM users WHERE username=?').get(USER); console.log('usuario olga creado (id '+u.id+', rol '+ROLE+')'); }
else console.log('usuario olga ya existe (id '+u.id+')');
